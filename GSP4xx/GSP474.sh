...
$(splitter)
In this task, we'll be creating a VPC ($VPC_NAME) with 2 subnets ($VPC_NAME-us-central1, $VPC_NAME-us-central1-ids) in us-east4 region.
EOF
pause
echo_cmd gcloud compute networks create $VPC_NAME --subnet-mode=custom
echo_cmd gcloud compute networks subnets create $VPC_NAME-us-central1 --region=$REGION --network=$VPC_NAME --range=172.21.0.0/24
echo_cmd gcloud compute networks subnets create $VPC_NAME-us-central1-ids --region=$REGION --network=$VPC_NAME --range=172.21.1.0/24
checkpoint
} # End of task 1
...
EOF
pause
echo_cmd gcloud compute routers create router-stamford-nat-west1 --network=$VPC_NAME --region=$REGION
echo_cmd gcloud compute routers nats create nat-gw-$VPC_NAME-west1 --router=router-stamford-nat-west1 --router-region=$REGION --auto-allocate-nat-external-ips --nat-all-subnet-ip-ranges
checkpoint
} # End of task 3
...
* Instance Template
    ...
    - template-$VPC_NAME-ids-$REGION
        ...
        Subnet: $VPC_NAME-us-central1-ids
        External IP: (Disabled)
* Managed Instance Group
    - mig-$VPC_NAME-web-us-central1
        Template: template-$VPC_NAME-web-us-central1
        Size: 2
        Zone: $REGION-a
    - mig-$VPC_NAME-ids-us-central1
        Template: template-$VPC_NAME-ids-us-central1
        Size: 1
        Zone: $REGION-a
EOF
pause
...
echo_cmd gcloud compute instance-groups managed create mig-$VPC_NAME-web-us-central1 --template=template-$VPC_NAME-web-$REGION --size=2 --zone=$REGION-a
...
echo_cmd gcloud compute instance-groups managed create mig-$VPC_NAME-ids-us-central1 --template=template-$VPC_NAME-ids-$REGION --size=1 --zone=$REGION-a
...
* Backend Service
    - be-$VPC_NAME-suricata-$REGION
        ...
        Protocol: TCP
        Backends:
            - Name: mig-$VPC_NAME-ids-us-central1
              Zone: $RESION-a
              Region: $REGION
* Forwarding Rule
...
echo_cmd gcloud compute backend-services add-backend be-$VPC_NAME-suricata-$REGION --instance-group=mig-$VPC_NAME-ids-us-central1 --instance-group-zone=$REGION-a --region=$REGION
echo_cmd gcloud compute forwarding-rules create ilb-$VPC_NAME-suricata-ilb-$REGION --load-balancing-scheme=INTERNAL --backend-service=be-$VPC_NAME-suricata-$REGION --is-mirroring-collector --network=$VPC_NAME --region=$REGION --subnet=$VPC_NAME-us-central1-ids --ip-protocol=TCP --ports=all

checkpoint
} # End of task 5

task6(){
IDS_INST_NAME=$(gcloud compute instance-groups managed list-instances mig-$VPC_NAME-ids-us-central1 --zone=$REGION-a  --format="csv(NAME)" --limit=1 | tail -n +2)
cat << EOF
...
echo_cmd gcloud compute packet-mirrorings create mirror-$VPC_NAME-web --collector-ilb=ilb-$VPC_NAME-suricata-ilb-$REGION --network=$VPC_NAME --mirrored-subnets=$VPC_NAME-us-central1 --region=$REGION
} # End of task 6
...
$(gcloud compute instances list --filter="name:mig-$VPC_NAME-web-us-central1*" --zones=$REGION-a --format='table(NAME, EXTERNAL_IP)')
...
task8(){
WEB_INST_NAME=$(gcloud compute instance-groups managed list-instances mig-$VPC_NAME-web-us-central1 --zone=$REGION-a --format="csv(NAME)" --limit=1 | tail -n +2)
